...
async function generateSocialImage(filename, title, siteUrl, siteEmail, authorImage, options = {}) {
	...
	if (!(title && targetDir && urlPath)) {
		console.error("eleventy-social-images Error: Missing values");
		return;
	}
	...
	let template = `<svg width="1200" height="628" viewbox="0 0 1200 628" xmlns="http://www.w3.org/2000/svg">
		...
		<g style="font-family:'cust',sans-serif">
			${svgTitle}
			<text x="265" y="480" fill="${titleColor}" font-size="30px" font-weight="700">${siteUrl}</text>
		</g>
		<g style="font-family:'cust',sans-serif">
		${svgTitle}
		<text x="265" y="520" fill="${titleColor}" font-size="30px" font-weight="700">${siteEmail}</text>
	</g>
	</svg>`;
	...
	try {
		...
		console.error("eleventy-social-images Error: ", err, { template, filename, title, siteUrl, siteEmail, authorImage, options } );
		return '';
	}
